...
now(function()
    ...
    if helpers.is_mac then
        mappings = {
            choose_in_split   = '<D-s>',
            choose_in_tabpage = '<D-t>',
            choose_in_vsplit  = '<D-v>',
    ...
    vim.keymap.set({ 'n', 'v' }, '<leader>pe', pickers.explorer, { desc = 'Open Pick explorer' })
    vim.keymap.set({ 'n', 'v' }, '<leader>pf', MiniPick.builtin.files, { desc = 'Open Pick files' })
